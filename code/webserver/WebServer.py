...
async def handle_client(reader, writer, fulfill_request):
    ...
    try:
        ...
        async def send_chunk(chunk, end_response=False):
            ...
            if (
                not hasattr(send_response, "headers_sent")
                or not send_response.headers_sent
            ):
            ...
            try:
                ...
                logger.warning(
                    f"[{request_id}] Connection lost while sending chunk: {str(e)}"
                )
                connection_alive = False
        ...
        if connection_alive:
            try:
                ...
                logger.error(
                    f"[{request_id}] Error in fulfill_request: {str(e)}", exc_info=True
                )

                if connection_alive and not (
                    hasattr(send_response, "headers_sent")
                    and send_response.headers_sent
                ):
                    try:
        ...
        logger.error(
            f"[{request_id}] Critical error handling request: {str(e)}", exc_info=True
        )
    finally:
...
async def start_server(
    ...
    if use_https or CONFIG.is_ssl_enabled():
        ...
        if not ssl_cert_file or not ssl_key_file:
            if CONFIG.is_ssl_enabled():
                raise ValueError(
                    "SSL is enabled in config but certificate/key files not found in environment"
                )
            else:
                raise ValueError(
                    "SSL certificate and key files must be provided for HTTPS"
                )
    ...
    server = await asyncio.start_server(
        lambda r, w: handle_client(r, w, fulfill_request), host, port, ssl=ssl_context
    )
    ...
    print(
        f"Serving {protocol} on {addr[0]} port {addr[1]} ({url_protocol}://{addr[0]}:{addr[1]}/) ..."
    )
    async with server:
...
async def fulfill_request(
    method, path, headers, query_params, body, send_response, send_chunk
):
    """
    ...
    try:
        ...
        if path == "/" or path == "":
            # Serve the news interface at the root
            try:
                await send_static_file(
                    "static/news_interface.html", send_response, send_chunk
                )  # Updated path
            except FileNotFoundError:
                # If news_interface.html doesn't exist, send a 404 error
                await send_response(404, {"Content-Type": "text/plain"})
                await send_chunk(
                    "News interface not found".encode("utf-8"), end_response=True
                )
            return
        elif (
            (path.find("html/") != -1)
            or path.find("static/") != -1
            or (path.find("png") != -1)
        ):
            ...
            await handle_mcp_request(
                query_params, body, send_response, send_chunk, streaming=use_streaming
            )
            return
            ...
            if not streaming:
                if generate_mode == "generate":
                    retval = await GenerateAnswer(
                        validated_query_params, None
                    ).runQuery()
                else:
            ...
            await send_response(404, {"Content-Type": "text/plain"})
            await send_chunk(
                f"No handler found for path: {path}".encode("utf-8"), end_response=True
            )
    except Exception as e:
...
if __name__ == "__main__":
    try:
        ...
        if is_azure:
            print(
                f"Running in Azure App Service: {os.environ.get('WEBSITE_SITE_NAME')}"
            )
