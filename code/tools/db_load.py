            ...
            async with session.get(url) as response:
                ...
                if ext is None:
                    ...
                    # For URLs ending with 'feed' or similar
                        ...
                        kw in path.lower() for kw in ["/feed", "/rss", "/podcast"]
                    ):
                        ext = ".xml"
        ...
        try:
            with open(file_path, "r", encoding="utf-8") as f:
                ...
                if len(parts) >= 3:
                    ...
                    if (
                        embedding_part.startswith("[") and embedding_part.endswith("]")
                    ) or (
                        embedding_part.replace(",", "")
                        .replace("-", "")
                        .replace(".", "")
                        .replace("e", "")
                        .replace("E", "")
...
async def process_csv_file(file_path: str, site: str) -> List[Dict[str, Any]]:
    ...
    try:
        with open(file_path, "r", encoding="utf-8", errors="replace") as csv_file:
            ...
            try:
                ...
                if len(csv_reader.fieldnames or []) < 2:
                    print(
                        "Warning: CSV file has fewer than 2 columns. This may not contain enough data."
                    )
            except Exception as e:
            ...
            for index, row in enumerate(csv_reader):
                try:
                    ...
                    if (index + 1) % 1000 == 0:
                        print(
                            f"Processed {index + 1} rows ({success_count} successful, {error_count} errors)"
                        )
                    ...
                    if (
                        error_count <= 5
                    ):  # Only print detailed traceback for the first few errors
                        traceback.print_exc()
                    elif error_count == 6:
        ...
        print(
            f"CSV processing complete: {success_count} rows processed successfully, {error_count} rows had errors"
        )
        return documents
        ...
        print(
            f"Returning {len(documents)} documents that were processed before the error"
        )
        return documents
...
async def loadJsonWithEmbeddingsToDB(
    file_path: str,
    site: str,
    batch_size: int = 100,
    ...
    try:
        ...
        if os.path.exists(file_path):
            ...
            if has_embeddings:
                ...
                print(
                    f"File exists and contains embeddings, using directly: {file_path}"
                )
        ...
        print(
            f"Loading data with embeddings from {resolved_path} for site {site} using database endpoint '{endpoint_name}'"
        )
            ...
            try:
                ...
                if len(batch_documents) >= batch_size or i == total_lines - 1:
                    if batch_documents:
                        ...
                        print(
                            f"Uploading batch {batch_idx + 1} of {total_batches} ({len(batch_documents)} documents)"
                        )
...
async def loadJsonToDB(
    ...
    try:
        ...
        print(
            f"Loading data from {original_path} (resolved to {resolved_path}) for site {site} using database endpoint '{endpoint_name}'"
        )
        ...
        if has_embeddings and not force_recompute:
            print(
                "File already contains embeddings, switching to direct loading mode..."
            )
            return await loadJsonWithEmbeddingsToDB(
                resolved_path, site, batch_size, delete_existing, endpoint_name
            )
        ...
        if os.path.exists(embeddings_path) and not force_recompute:
            ...
            if sys.stdin.isatty():
                response = input(
                    f"A file with embeddings already exists at {embeddings_path}. Use it? (y/n): "
                )
            ...
            if use_existing:
                ...
                return await loadJsonWithEmbeddingsToDB(
                    embeddings_path, site, batch_size, delete_existing, endpoint_name
                )
        ...
        if (
            ...
            and is_url_path
            and (
                "/feed" in original_path.lower()
                or "/rss" in original_path.lower()
                or "/podcast" in original_path.lower()
                or original_path.lower().endswith(("feed", "rss", "podcast", "xml"))
            )
        ):
            print(
                "XML file from URL looks like it might be an RSS feed. Processing as RSS..."
            )
            file_type = "rss"
        ...
        if file_type == "csv":
        ...
        elif file_type == "rss" or (
            file_type == "xml"
            and ("/feed" in original_path.lower() or "/rss" in original_path.lower())
        ):
            ...
            if json_only_format:
                print(
                    "Detected JSON-only format. URLs will be extracted from within the JSON data."
                )
            ...
            with open(embeddings_path, "w", encoding="utf-8") as embed_file:
                ...
                for i in range(0, len(all_documents), batch_size):
                    ...
                    if batch_docs and batch_texts:
                        try:
                            print(
                                f"Computing embeddings for batch of {len(batch_texts)} texts"
                            )

                            # Compute embeddings for the batch
                            embeddings = await batch_get_embeddings(
                                batch_texts, provider, model
                            )
                            ...
                            for j, embedding in enumerate(embeddings):
                                if j < len(batch_docs):
                                    doc = batch_docs[
                                        j
                                    ].copy()  # Create a copy of the document
                                    ...
                                    embed_file.write(
                                        f"{doc['url']}\t{doc_json}\t{embedding_str}\n"
                                    )
                            ...
                            batch_idx = i // batch_size
                            total_batches = (
                                len(all_documents) + batch_size - 1
                            ) // batch_size

                            print(
                                f"Uploading batch {batch_idx + 1} of {total_batches} ({len(docs_with_embeddings)} documents)"
                            )
                    ...
                    # Print progress
                    print(
                        f"Processed {min(i + batch_size, len(all_documents))}/{len(all_documents)} documents"
                    )

            print(
                f"Loading completed. Added {total_documents} documents to the database."
            )
...
async def loadUrlListToDB(
    ...
    try:
        ...
        print(
            f"Found {total_valid_urls} valid URLs out of {total_urls} lines in the file"
        )
            ...
            try:
                ...
                try:
                    ...
                    if file_type == "rss" or file_type == "xml":
                        ...
                        print(
                            f"Warning: Unsupported file type for URL {url}: {file_type}"
                        )
...
async def main():
    ...
    if not await is_url(args.file_path) and not os.path.exists(args.file_path):
        print(
            f"Warning: File not found at '{args.file_path}'. Will try to resolve or download it."
        )
    ...
    try:
        ...
        if os.path.exists(file_path):
            file_type, has_embeddings = await detect_file_type(file_path)
            print(
                f"Detected file type: {file_type}, contains embeddings: {'Yes' if has_embeddings else 'No'}"
            )
            ...
            if has_embeddings and not args.force_recompute:
                ...
                await loadJsonWithEmbeddingsToDB(
                    file_path,
                    args.site,
                    args.batch_size,
                    args.delete_site,
