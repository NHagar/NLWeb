...
async def serve(
    server_url: str = DEFAULT_SERVER_URL, endpoint: str = DEFAULT_ENDPOINT
) -> None:
    """
    ...
    """
    print(
        f"Starting NLWeb MCP interface - connecting to {server_url}{endpoint}",
        file=sys.stderr,
    )
    ...
    async def call_tool(name: str, arguments: dict) -> list[TextContent]:
        ...
        try:
            ...
            return [
                TextContent(type="text", text=f"Error processing response: {str(e)}")
            ]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--endpoint", default=DEFAULT_ENDPOINT, help="NLWeb server endpoint"
    )
