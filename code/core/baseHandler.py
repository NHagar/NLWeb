...
from utils.utils import get_param, log, siteToItemType
...
class NLWebHandler:
    def __init__(self, query_params, http_handler):
        ...
        self.decontextualized_query = get_param(
            query_params, "decontextualized_query", str, ""
        )
        ...
        self.context_description = get_param(
            query_params, "context_description", str, ""
        )
        ...
        logger.info("NLWebHandler initialized with parameters:")
        ...
        log(
            f"NLWebHandler initialized with site: {self.site}, query: {self.query}, prev_queries: {self.prev_queries}, mode: {self.generate_mode}, query_id: {self.query_id}, context_url: {self.context_url}"
        )
    ...
    async def send_message(self, message):
        logger.debug(
            f"Sending message of type: {message.get('message_type', 'unknown')}"
        )
        async with self._send_lock:  # Protect send operation with lock
            ...
            if self.streaming and self.http_handler is not None:
                ...
                try:
                    ...
                    logger.debug("Message streamed successfully")
                ...
                logger.debug("Message added to return value store")
    ...
    async def runQuery(self):
        ...
        try:
            ...
            if self.query_done:
                ...
                log("query done prematurely")
                return self.return_value
            if not self.fastTrackWorked:
                logger.info("Fast track did not work, proceeding with normal ranking")
                log("Going to get ranked answers")
                await self.get_ranked_answers()
                log("ranked answers done")
    ...
    async def prepare(self):
        ...
        tasks.append(
            asyncio.create_task(analyze_query.DetectMultiItemTypeQuery(self).do())
        )
        ...
        if not self.retrieval_done_event.is_set():
            logger.info(
                "Retrieval not done by fast track, performing regular retrieval"
            )
    ...
    def decontextualizeQuery(self):
        ...
        if len(self.prev_queries) < 1:
            ...
            logger.debug(
                "Decontextualized query already provided - using NoOpDecontextualizer"
            )
            return decontextualize.NoOpDecontextualizer(self)
        elif len(self.prev_queries) > 0:
            logger.debug(
                f"Using PrevQueryDecontextualizer with {len(self.prev_queries)} previous queries"
            )
            return decontextualize.PrevQueryDecontextualizer(self)
        elif len(self.context_url) > 4 and len(self.prev_queries) == 0:
            logger.debug(
                f"Using ContextUrlDecontextualizer with context URL: {self.context_url}"
            )
            return decontextualize.ContextUrlDecontextualizer(self)
        else:
            logger.debug(
                "Using FullDecontextualizer with both context URL and previous queries"
            )
    ...
    async def get_ranked_answers(self):
        try:
            logger.info(
                f"Starting ranking process on {len(self.final_retrieved_items)} items"
            )
            log(f"Getting ranked answers on {len(self.final_retrieved_items)} items")
            await ranking.Ranking(
                self, self.final_retrieved_items, ranking.Ranking.REGULAR_TRACK
            ).do()
